...
void Game::response(std::string side){
    ...
    if (ball->dx > 0)
    {
        if (ball->dy > 0)
        {
            if (side == "Left" or side == "Bottom") mulx = -1;
                else muly = -1;
        }
        else if (ball->dy < 0) 
        {
            if (side == "Left" or side == "Top") mulx = -1;
                else muly = -1;
        }
    }
    else if (ball->dx < 0)
    {
        if (ball->dy > 0)
        {
            if (side == "Right" or side == "Bottom") mulx = -1;
                else muly = -1;
        }
        else if (ball->dy < 0) 
        {
            if (side == "Left" or side == "Right") mulx = -1;
                else muly = -1;
        }
    }
    ball->setDirection(mulx * ball->dx, muly * ball->dy);
}
